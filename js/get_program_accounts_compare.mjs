...
async function getPubkeys () {
  ...
  const fetchAccounts = keys.map(async (key) => {
    ...
    try {
      ...
      // NOTE: You may need to add/remove filters below depending on your params.
  ...
  // wait for the results of getProgramAccounts and return the final value of pubkeys
  return pubkeys;
}
