...
async function getAssets() {
  ...
  const fetchAssets = keys.map(async (key) => {
    ...
    nfts[key] = await umi.rpc.getAssetsByOwner({
      owner: umiPublicKey,
      page: 0,
      pageLimit: 1000,
    });
...
getAssets()
  .then((nfts) => {
    ...
    console.log(
      `\n${diff_1.length} assets in ${endpoints[keys[0]]} that are not in ${
        endpoints[keys[1]]
      }:`
    );
    ...
    console.log(
      `\n${diff_2.length} assets in ${endpoints[keys[1]]} that are not in ${
        endpoints[keys[0]]
      }:`
    );
